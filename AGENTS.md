...
This repository is an ESP-IDF firmware project for `esp32`. The root [CMakeLists.txt](CMakeLists.txt) defines the project, and [main/main.cpp](main/main.cpp) contains the `app_main()` entry point built by ESP-IDF. Use [main/CMakeLists.txt](main/CMakeLists.txt) to register additional source files as the firmware grows. The archived [archive/arduino/gps.ino](archive/arduino/gps.ino) and [archive/arduino/gps_light.ino](archive/arduino/gps_light.ino) sketches are historical references and are not part of the current CMake build. Container and editor setup lives in `.devcontainer/` and `.vscode/`.
